// -*-c++-*-
/*
 *Copyright: Copyright (C) Hiroki SHIMORA, Hidehisa AKIYAMA
...
/*!
 */
bool SimplePassChecker::operator()(const PredictState &state,
                                   ...
                                   const double &first_ball_speed) const
{
    ...
    if (from.isGhost() || to.isGhost() || from.posCount() > VALID_TEAMMATE_ACCURACY || to.posCount() > VALID_TEAMMATE_ACCURACY)
    {
        return false;
    }

    const Vector2D from_pos = (from.isSelf() ? state.ball().pos() : from.pos());
    ...
    if (receive_point.x <= ServerParam::i().ourPenaltyAreaLineX() + 3.0 && receive_point.absY() <= ServerParam::i().penaltyAreaHalfWidth() + 3.0)
    {
#ifdef DEBUG_PRINT
        dlog.addText(Logger::PASS,
    ...
    if (receive_point.absX() >= ServerParam::i().pitchHalfLength() || receive_point.absY() >= ServerParam::i().pitchHalfWidth())
    {
#ifdef DEBUG_PRINT
        dlog.addText(Logger::PASS,
    ...
    if (to.goalie() && receive_point.x < ServerParam::i().ourPenaltyAreaLineX() + 1.0 && receive_point.absY() < ServerParam::i().penaltyAreaHalfWidth() + 1.0)
    {
#ifdef DEBUG_PRINT
        dlog.addText(Logger::PASS,
    ...
    if (to.goalie() && receive_point.x > state.ourDefenseLineX() - 3.0)
    {
#ifdef DEBUG_PRINT
        dlog.addText(Logger::PASS,
    ...
    const double OVER_TEAMMATE_IGNORE_DISTANCE2 = std::pow(pass_dist + (receive_point.x >= +25.0 ? 2.0 : 6.0), 2);
         ...
         ++o)
    {
        ...
        if (from.isSelf())
        {
            ...
            const double hide_radian = std::asin(std::min(control_area / std::sqrt(opp_dist2), 1.0));
    ...
    if (from.pos().x >= +25.0 && to.pos().x >= +25.0)
    {
