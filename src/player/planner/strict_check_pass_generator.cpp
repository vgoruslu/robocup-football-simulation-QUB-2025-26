...
void StrictCheckPassGenerator::updateReceivers(const WorldModel &wm)
{
         ...
         ++p)
    {
        ...
        if (is_self_passer)
        {
            ...
            if ((*p)->goalie() && (*p)->pos().x < SP.ourPenaltyAreaLineX() + 15.0)
            {
                continue;
            }
        }
                                                ...
                                                const Receiver &receiver)
{
    ...
    static const double MIN_DIRECT_PASS_DIST = ServerParam::i().defaultKickableArea() * 2.2;
    static const double MAX_DIRECT_PASS_DIST = 0.8 * inertia_final_distance(ServerParam::i().ballSpeedMax(),
                                                                            ServerParam::i().ballDecay());
    static const double MAX_RECEIVE_BALL_SPEED = ServerParam::i().ballSpeedMax() * std::pow(ServerParam::i().ballDecay(), MIN_RECEIVE_STEP);
    ...
    //
    if (receiver.pos_.x > SP.pitchHalfLength() - 1.5 || receiver.pos_.x < -SP.pitchHalfLength() + 5.0 || receiver.pos_.absY() > SP.pitchHalfWidth() - 1.5)
    {
    ...
    //
    if (receiver.pos_.x < M_first_point.x + 1.0 && receiver.pos_.dist2(SP.ourTeamGoalPos()) < std::pow(18.0, 2))
    {
#ifdef DEBUG_DIRECT_PASS
        dlog.addText(Logger::PASS,
    ...
    if (ball_move_dist < MIN_DIRECT_PASS_DIST || MAX_DIRECT_PASS_DIST < ball_move_dist)
    {
#ifdef DEBUG_DIRECT_PASS
        dlog.addText(Logger::PASS,
    ...
    if (wm.gameMode().type() == GameMode::GoalKick_ && receive_point.x < SP.ourPenaltyAreaLineX() + 1.0 && receive_point.absY() < SP.penaltyAreaHalfWidth() + 1.0)
    {
#ifdef DEBUG_DIRECT_PASS
        dlog.addText(Logger::PASS,
    ...
    const double max_receive_ball_speed = std::min(MAX_RECEIVE_BALL_SPEED,
                                                   ptype->kickableArea() + (SP.maxDashPower() * ptype->dashPowerRate() * ptype->effortMax()) * 1.8);
                                                 ...
                                                 const Receiver &receiver)
{
    ...
    static const double MAX_LEADING_PASS_DIST = 0.8 * inertia_final_distance(ServerParam::i().ballSpeedMax(),
                                                                             ServerParam::i().ballDecay());
    static const double MAX_RECEIVE_BALL_SPEED = ServerParam::i().ballSpeedMax() * std::pow(ServerParam::i().ballDecay(), MIN_RECEIVE_STEP);
    ...
    const double max_receive_ball_speed = std::min(MAX_RECEIVE_BALL_SPEED,
                                                   ptype->kickableArea() + (SP.maxDashPower() * ptype->dashPowerRate() * ptype->effortMax()) * 1.5);
    ...
    for (int d = 1; d <= DIST_DIVS; ++d)
    {
        ...
        for (int a = 0; a < ANGLE_DIVS; a += a_step)
        {
            ...
            const Vector2D receive_point = receiver.inertia_pos_ + Vector2D::from_polar(player_move_dist, angle);
            ...
            if (receive_point.x > SP.pitchHalfLength() - 3.0 || receive_point.x < -SP.pitchHalfLength() + 5.0 || receive_point.absY() > SP.pitchHalfWidth() - 3.0)
            {
#ifdef DEBUG_LEADING_PASS
                dlog.addText(Logger::PASS,
            ...
            if (receive_point.x < M_first_point.x && receive_point.dist2(our_goal) < OUR_GOAL_DIST_THR2)
            {
#ifdef DEBUG_LEADING_PASS
                dlog.addText(Logger::PASS,
            ...
            if (ball_move_dist < MIN_LEADING_PASS_DIST || MAX_LEADING_PASS_DIST < ball_move_dist)
            {
#ifdef DEBUG_LEADING_PASS
                dlog.addText(Logger::PASS,
                                                 ...
                                                 const Receiver &receiver)
{
    ...
    static const double MAX_THROUGH_PASS_DIST = 0.9 * inertia_final_distance(ServerParam::i().ballSpeedMax(),
                                                                             ServerParam::i().ballDecay());
    static const double MAX_RECEIVE_BALL_SPEED = ServerParam::i().ballSpeedMax() * std::pow(ServerParam::i().ballDecay(), MIN_RECEIVE_STEP);
    ...
    if (receiver.pos_.x < min_receive_x - MAX_MOVE_DIST || receiver.pos_.x < 1.0)
    {
    ...
    for (int a = 0; a <= ANGLE_DIVS; ++a)
    {
             ...
             move_dist += MOVE_DIST_STEP)
        {
            ...
            const Vector2D receive_point = receiver.inertia_pos_ + unit_rvec * move_dist;
            ...
            if (receive_point.x > SP.pitchHalfLength() - 1.5 || receive_point.absY() > SP.pitchHalfWidth() - 1.5)
            {
#ifdef DEBUG_THROUGH_PASS
                dlog.addText(Logger::PASS,
            ...
            if (ball_move_dist < MIN_THROUGH_PASS_DIST || MAX_THROUGH_PASS_DIST < ball_move_dist)
            {
#ifdef DEBUG_THROUGH_PASS
                dlog.addText(Logger::PASS,
            ...
            else if (receiver.speed_ > 0.2 && (receiver_vel_angle - angle).abs() < 15.0)
            {
#ifdef DEBUG_THROUGH_PASS
                dlog.addText(Logger::PASS,
...
#endif
                start_step += 1;
                if ((receive_point.x > SP.pitchHalfLength() - 5.0 || receive_point.absY() > SP.pitchHalfWidth() - 5.0) && ball_move_angle.abs() > 30.0 && start_step >= 10)
                {
                    start_step += 1;
                }
            }
                                                ...
                                                const char *description)
{
    ...
    for (int step = min_step; step <= max_step; ++step)
    {
        ...
        if (M_pass_type == 'T')
        {
            ...
            if (receive_point.x > 30.0 && step >= 15 && (!opponent || !opponent->goalie()) && o_step >= step) // Magic Number
            {
 ...
 */
int StrictCheckPassGenerator::predictReceiverReachStep(const Receiver &receiver,
                                                       const Vector2D &pos,
                                                       const bool use_penalty)
{
    ...
    if (M_pass_type == 'L')
    {
        ...
        if (dash_angle.abs() > 90.0 || receiver.player_->bodyCount() > 1 || (dash_angle - receiver.player_->body()).abs() > 30.0)
        {
            n_turn += 1;
        }
    }
                                                        ...
                                                        const AbstractPlayerObject **opponent)
{
         ...
         ++o)
    {
        ...
        if (step < min_step || (step == min_step && o->bonus_distance_ > bonus_dist))
        {
                                                       ...
                                                       const int max_cycle)
{
    ...
    for (int cycle = std::max(1, min_cycle); cycle <= max_cycle; ++cycle)
    {
        const Vector2D ball_pos = inertia_n_step_point(first_ball_pos,
                                                       ...
                                                       cycle,
                                                       SP.ballDecay());
        const double control_area = (opponent.player_->goalie() && penalty_area.contains(ball_pos)
                                         ? SP.catchableArea()
                                         : ptype->kickableArea());
        ...
        if (M_pass_type == 'T' && first_ball_vel.x > 2.0 && (receive_point.x > wm.offsideLineX() || receive_point.x > 30.0))
        {
#if 0
            dlog.addText( Logger::PASS,
                          "__ step=%d no bonus",
        ...
        if (dash_dist > ptype->realSpeedMax() * (cycle + std::min(opponent.player_->posCount(), 5)))
        {
#if 0
            dlog.addText( Logger::PASS,
