// -*-c++-*-
/*
 *Copyright: Copyright (C) Hiroki SHIMORA
...
/*-------------------------------------------------------------------*/
/*! */
double s_get_ball_speed_for_pass(const double &distance)
{
    if (distance >= 20.0) {
        // return 3.0;
        return 2.5;
...
bool isNearAttackingCornerFlag(const Vector2D &p)
{
    return p.x > ServerParam::i().pitchHalfLength() - 6.0
        && std::fabs(p.y) > ServerParam::i().pitchHalfWidth() - 6.0;
}

bool isInAttackingCornerChannel(const Vector2D &p)
{
    return p.x > ServerParam::i().pitchHalfLength() - 18.0
        && std::fabs(p.y) > ServerParam::i().pitchHalfWidth() - 18.0;
}
...
/*-------------------------------------------------------------------*/
/*! */
void ActGen_DirectPass::generate(
...
) const
{
...
#ifdef DEBUG_PRINT
    ActionChainGraph::writeChainInfoLog(
        "branch cut checking: history = ", current_wm, path, 0.0);
#endif
        ...
        if (c == 0) {
            ...
            if (teammate_min < self_min) {
                const PlayerObject *teammate =
                    current_wm.interceptTable().firstTeammate();
            ...
            ball_holder_unum =
                path[c - 1].state().ballHolder()->unum();
        }
    ...
    const bool attacking_corner_channel =
        isInAttackingCornerChannel(holder->pos());

    const SimplePassChecker pass_check;

    int generated_count = 0;

    for (auto it = state.ourPlayers().begin(),
         ...
         it != end; ++it)
    {
        ...
        if (attacking_corner_channel
            && isNearAttackingCornerFlag((*it)->pos()))
        {
#ifdef DEBUG_PRINT
            dlog.addText(Logger::ACTION_CHAIN,
                         "direct: ignored corner-flag return pass to %d",
                         (*it)->unum());
#endif
            continue;
        }
...
#ifdef DEBUG_PRINT
            dlog.addText(Logger::ACTION_CHAIN,
                         "direct: can't pass from %d to %d(%.1f %.1f), target accuracy low",
                         holder->unum(),
                         (*it)->unum(),
                         (*it)->pos().x,
                         (*it)->pos().y);
        ...
        PredictState::ConstPtr result_state(
        ...
        result->push_back(
            ActionStatePair(action, result_state));
    }
